...
def get_select_data(request):
    ...
    if selected_faculty:
        groups = TimeTable.objects.filter(Faculty=selected_faculty).values('SGroup').distinct().order_by('SGroup')
    else:
        groups = TimeTable.objects.values('SGroup').distinct().order_by('SGroup')

    data = {
        'groups': [group['SGroup'] for group in groups],
    }
